...
const Board = ({ grid, selectedCell, onCellSelect, status }: BoardProps) => {
  ...
  return (
    <div
      className="bg-white/92 p-2.5 sm:p-5 rounded-[28px] sm:rounded-[40px] shadow-soft border-2 sm:border-4 border-white/80 select-none relative max-w-[95vw] mx-auto"
      role="grid"
      aria-label="Mini Sudoku board"
    >
      <div className="absolute left-1/2 top-3 bottom-3 sm:top-6 sm:bottom-6 w-[3px] sm:w-[4px] bg-brand-100/80 transform -translate-x-1/2 z-0 rounded-full"></div>
      <div className="absolute top-1/2 left-3 right-3 sm:left-6 sm:right-6 h-[3px] sm:h-[4px] bg-brand-100/80 transform -translate-y-1/2 z-0 rounded-full"></div>

      <div className="grid grid-cols-2 gap-2.5 sm:gap-6 relative z-10">
        {quadrants.map((quad, qIdx) => (
          <div key={qIdx} className="grid grid-cols-2 gap-1.5 sm:gap-3" role="rowgroup">
            {quad.map(([r, c]) => {
              const cell = grid[r][c];
...
const Controls = ({
  ...
  return (
    <div className="w-full max-w-sm mt-5 sm:mt-8 space-y-3.5 sm:space-y-6">
      <div className={`grid grid-cols-5 gap-2 sm:gap-3 transition-opacity duration-300 ${!isPlaying ? 'opacity-30 pointer-events-none' : ''}`}>
        {[1, 2, 3, 4].map((num) => (
          <button
            key={num}
            onClick={() => onInput(num)}
            className="aspect-square min-h-[3.1rem] bg-white border-b-4 sm:border-b-8 border-slate-200 rounded-xl sm:rounded-[24px] text-xl sm:text-3xl font-black text-slate-700 active:border-b-0 active:translate-y-1 sm:active:translate-y-2 transition-all shadow-sm hover:bg-slate-50 btn-press"
          >
            {num}
          </button>
        ))}
        <button
          onClick={() => onInput(0)}
          className="aspect-square min-h-[3.1rem] bg-slate-100 border-b-4 sm:border-b-8 border-slate-300 rounded-xl sm:rounded-[24px] text-slate-500 active:border-b-0 active:translate-y-1 sm:active:translate-y-2 transition-all shadow-sm hover:bg-slate-200 flex items-center justify-center btn-press"
        >
      ...
      <div className="flex flex-col gap-3">
        <div className={actionGridClass}>
          <button
            onClick={onCheck}
            disabled={!isPlaying}
            className="min-h-[3.4rem] flex items-center justify-center gap-2 sm:gap-3 bg-brand-500 hover:bg-brand-600 text-white py-3.5 sm:py-5 rounded-2xl sm:rounded-[28px] text-base sm:text-xl font-black shadow-lg shadow-brand-500/20 active:scale-95 transition-all disabled:opacity-50 btn-press"
          >
          ...
          {canUseHint ? (
            <button
              ...
              className={`min-h-[3.4rem] flex items-center justify-center gap-2 sm:gap-3 py-3.5 sm:py-5 rounded-2xl sm:rounded-[28px] text-base sm:text-xl font-black active:scale-95 transition-all btn-press border-b-4 sm:border-b-8 ${
                hintUsed
        ...
        <button
          onClick={onGiveUp}
          disabled={!isPlaying}
          className="min-h-[3rem] flex items-center justify-center gap-2 py-3 rounded-xl sm:rounded-2xl text-slate-500 hover:text-slate-700 font-bold active:scale-95 transition-all btn-press border-2 border-slate-100 hover:bg-white disabled:opacity-30"
        >
...
export default function App() {
  ...
  return (
    <div className="w-full max-w-[26rem] sm:max-w-xl flex flex-col items-center app-container px-3 sm:px-0">
      <div className="text-center mb-5 sm:mb-10 w-full header-section">
        <div className="flex flex-col items-center justify-center gap-2 mb-3 sm:mb-4">
          ...
          <p className="text-brand-700/90 font-bold text-sm sm:text-base max-w-xs sm:max-w-none">The Number Puzzle Adventure!</p>
        </div>

        <div className="bg-brand-100/90 p-1.5 rounded-2xl sm:rounded-[28px] flex w-full shadow-sm">
        ...
        <div className="mt-3 min-h-14 rounded-2xl bg-white/80 border border-white/80 px-4 py-3 text-left shadow-sm">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-brand-700/60">{difficulty.split(' ')[0]} mode</p>
          <p className="mt-1 text-sm sm:text-[15px] font-bold text-slate-600">{difficultyHelperText}</p>
        </div>
      </div>
      ...
      <div className="w-full">
        ...
        <div className="mt-3 min-h-[58px] sm:min-h-[76px] flex items-center justify-center">
          <div
            className={`w-full max-w-sm px-4 sm:px-6 py-2.5 sm:py-3 rounded-[22px] sm:rounded-full text-sm sm:text-lg font-black shadow-xl flex items-center justify-center text-center gap-2 sm:gap-3 border-2 sm:border-4 transition-all duration-300 ${
      ...
      <div className="mt-7 sm:mt-10 text-brand-900/30 text-[11px] sm:text-sm font-black tracking-[0.2em] uppercase flex flex-wrap justify-center gap-2 sm:gap-4 mb-3 sm:mb-4">
